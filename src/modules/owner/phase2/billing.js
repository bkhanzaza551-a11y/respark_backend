...
import { attachBranchStock, createStockMovement, convertConsumableQty, normalizeBranchId, toAmount } from "../../../lib/phase2.js";
          ...
          if (removed.itemType === "PRODUCT" && removed.productId) {
            ...
            if (svc && svc.consumables && svc.consumables.length > 0) {
              ...
              for (const cons of matchedConsumables.length > 0 ? matchedConsumables : svc.consumables.filter(c => c.variation == null)) {
                const returnQty = convertConsumableQty(Number(cons.reqdQty), cons.product) * Number(removed.qty || 1);
                await createStockMovement(tx, {
                  salonId: req.salonId,
                  branchId: existingInvoice.branchId,
            ...
            if (existingItem && existingItem.itemType === "PRODUCT" && existingItem.productId) {
              ...
              if (newQty !== oldQty) {
                ...
                if (svc && svc.consumables && svc.consumables.length > 0) {
                  ...
                  const matchedConsumables = svc.consumables.filter(c => c.variation === serviceVariation || (serviceVariation == null && c.variation == null));
                  for (const cons of matchedConsumables.length > 0 ? matchedConsumables : svc.consumables.filter(c => c.variation == null)) {
                    const rawQtyDiff = (newQty - oldQty) * Number(cons.reqdQty);
                    const qtyDiff = convertConsumableQty(rawQtyDiff, cons.product);
                    if (qtyDiff < 0) {
                      await createStockMovement(tx, {
                        salonId: req.salonId,
                        branchId: existingInvoice.branchId,
        ...
        for (const item of invoice.items) {
          if (item.itemType === "SERVICE" && item.serviceId) {
            ...
            if (svc && svc.consumables && svc.consumables.length > 0) {
              ...
              const matchedConsumables = svc.consumables.filter(c => c.variation === serviceVariation || (serviceVariation == null && c.variation == null));
              for (const cons of matchedConsumables.length > 0 ? matchedConsumables : svc.consumables.filter(c => c.variation == null)) {
                const rawQty = Number(cons.reqdQty);
                const qtyToDeduct = convertConsumableQty(rawQty, cons.product);
                if (qtyToDeduct > 0) {
                  await createStockMovement(tx, {
                    salonId: req.salonId,
                    ...
                    referenceId: invoice.id,
                    note: `Predefined consumable: ${cons.product?.name || ""} (${rawQty} ${cons.product?.unit || ""})`,
                    allowNegativeStock: true
                  });
                }
              }
